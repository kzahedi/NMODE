...
DataPopulationModuleNode::DataPopulationModuleNode(DataNode *parent)
  : DataNode(parent)
{ }

void DataPopulationModuleNode::add(DataParseElement *element)
{
  if(element->closing(TAG_POPULATION_MODULE_NODE))
  {
    current = parent;
    return;
  }
  ...
  if(element->opening(TAG_POPULATION_MODULE_NODE))
  {
    ...
    VLOG(100) << "set label and type to " << _label << ", " << _type;
  }

  if(element->opening(TAG_BIAS))
  {
    element->set(TAG_VALUE,  _bias);
    VLOG(100) << "set bias to " << _bias;
  }
  ...
  if(element->opening(TAG_POSITION))
  {
    ...
    VLOG(100) << "set position to " << _position;
  }
...
void DataPopulationModuleNode::createXsd(XsdSpecification *spec)
{
  XsdSequence *root = new XsdSequence(TAG_POPULATION_MODULE_NODE_DEFINITION);
  ...
  root->add(NE(TAG_BIAS,             TAG_BIAS_DEFINITION,             1, 1));
  spec->add(root);
...
string DataPopulationModuleNode::type()
{
  return _type;
}

string DataPopulationModuleNode::label()
{
  return _label;
}

P3D DataPopulationModuleNode::position()
{
  return _position;
}

string DataPopulationModuleNode::transferfunction()
{
