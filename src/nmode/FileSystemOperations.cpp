...
void FileSystemOperations::createDir(string dirName) throw (NMODEException)
{
...
bool FileSystemOperations::doesExecutableExist(string exe) throw (NMODEException)
{
  fs::path path;
  string path_string = string(getenv("PATH"));
  if (path_string.length() == 0) throw NMODEException("Cannot read PATH system variable.");
